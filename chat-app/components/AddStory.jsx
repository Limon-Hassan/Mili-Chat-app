...
export default function AddStory({ onClose = () => {}, onSave = () => {} }) {
  ...
  const handleSave = async () => {
    ...
    if (videoFile) {
      onSave(
        videoFile,
        `story-${Date.now()}.${videoFile.name.split('.').pop()}`,
      );
      setIsSaving(false);
      setProgress(100);
    ...
    let duration = imageFile ? DEFAULT_DURATION : DEFAULT_VIDEO_DURATION;
    ...
    const drawLoop = now => {
      ...
      setProgress(Math.min(100, (elapsed / duration) * 100));
      if (elapsed < duration) rafId = requestAnimationFrame(drawLoop);
      else {
        try {
          recorder.stop();
        } catch {}
  ...
  return (
    <div className="fixed w-full inset-0 z-50 flex items-center justify-center bg-black/70 mobile:p-0 tablet:p-0 laptop:p-0 computer:p-4">
      <div
        ref={editorRef}
        className="relative mobile:w-full mobile:h-full tablet:w-full tablet:h-screen laptop:w-full laptop:h-screen computer:max-w-180 computer:h-180 bg-black rounded-xl overflow-hidden shadow-2xl"
      >
        <div className="absolute top-3 left-3 z-40 flex gap-2">
          <label className="cursor-pointer text-white/90 bg-white/10 px-2 py-1 rounded">
            <input
        ...
        <div className="absolute top-3 right-3 z-40 flex gap-2">
          ...
          <button
            onClick={handleSave}
            disabled={
              isSaving || (!videoFile && !imageFile && overlays.length === 0)
            }
            className="bg-green-600 text-white px-3 py-1 rounded flex items-center gap-2"
          >
            <Save size={14} />{' '}
        ...
        <div className="w-full h-full relative">
          ...
          {!videoFile && !imageFile && (
            <div
              className={`absolute inset-0 ${bg === 'radiant' ? 'bg-linear-to-br from-purple-500 to-pink-500' : bg === 'red' ? 'bg-red-600' : bg === 'blue' ? 'bg-blue-600' : 'bg-black'}`}
            ></div>
          )}
          {overlays.map(o => (
            <div
