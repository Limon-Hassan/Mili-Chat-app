...
const BlockMenu = ({ setBlockMenu, chatUserId }) => {
  ...
  let handleMsgBlock = async () => {
    ...
    MsgblockUser(blockedUserId: $blockedUserId) {
      ...
      let data = await request(mutation, { blockedUserId: chatUserId });
      setBlockMenu(false);
      console.log(data);
    } catch (error) {
      console.log(error);
  ...
  `;
      const data = await request(query, { blockerId: chatUserId });
      setBlockMenu(false);
      console.log(data);
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      <div className="absolute top-1/2 translate-y-[-50%] left-1/2 translate-x-[-50%] w-full h-full bg-black/50 flex items-center justify-center">
        <div className=" relative w-80 h-auto bg-white flex flex-col items-center justify-center gap-2 p-2 rounded-md ring-5 ring-gray-400">
