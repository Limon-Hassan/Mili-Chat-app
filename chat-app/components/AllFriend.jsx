import React from 'react';

const AllFriend = () => {
  return (
    <>
      <section>
        <div>
          <h2 className="text-2xl flex items-center justify-between font-bold font-inter text-white mb-5">
            Friends
            <span className="text-[16px] font-inter font-semibold cursor-pointer underline active:text-blue-500">
              see all
            </span>
          </h2>
          <div className="grid grid-cols-6 items-center gap-2.5 border border-gray-400 p-5 rounded-lg">
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
            <div className="flex flex-col items-center justify-center p-2.5 border border-gray-300 rounded-lg mx-auto">
              <img
                className="w-[100px] h-[100px] object-cover rounded-md"
                src="/Image.jpg"
                alt="User"
              />
              <h4 className="text-sm text-center mx-auto font-inter font-medium mt-2">
                mahammud hassan limon
              </h4>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default AllFriend;
